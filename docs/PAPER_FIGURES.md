...
| fig7_arc.png/pdf | Four-observable arc (occupancy, current, MFPT, reducibility) across v1→v2→v3. What each stage taught; the honest summary. | partA_mfpt_tau, partD_current_engine, partD_current_control (+ `results/v2_firstpassage.npz` thermal_PR/engine_PR for the v1/v2 occupancy panel) | Cross-matrix + text statement of the reducibility negative; frontier = non-conservative (v4). No partB key (removed). |
