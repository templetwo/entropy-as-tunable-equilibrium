# v4.4 Scout — Decision Rule v3.4 (whole-object re-pass revision of v3.3, for re-review + ratification)

*HQ (Mac Studio Claude Code seat, overwatch/provenance), 2026-07-08. **This is a DRAFT for re-review (ChatGPT methodology) + Antigravity re-audit + Anthony final-say. It is NOT registered, NOT chronicled, and edits NO harness bytes.** It revises `v44_scout_DECISION_RULE_v3.3.md` (a draft, not frozen) to clear the v3.3 whole-object re-pass — three independent audit lenses returned **one BLOCKER** (the omega veto is throttled by the primary's ladder: omega was stamped once at the primary's terminal block count, so the veto as actually implemented misses a real +2×floor omega current ~21–24% per cell and the measured gate-8 rate ≈ 0.0075 BREACHES the design's own ≤ 0.005 tolerance while §9.3 asserted "≲ 10⁻³" without computing it) plus **eight majors** (omega sidedness undefined and internally inconsistent with §13's arithmetic; the `occupancy_x` statistic — the ONLY statistic that ever carried a confirmed v4.x effect — recorded but never banded and holding no veto; the omega-ANOMALOUS disposition path un-powered, re-importing the exact evidence-destruction defect v3.3 repaired in §8; `pivot_licensed()` completeness asserted for the gating dict only, so the silent-shrink leak recurs on `omega_bands`/`descriptive_bands`; the STABLE-coupling sweep all-positive, certifying the suspenders in a regime where transduction inversion makes them provably inert; and gate-8's expected number never computed). The frozen v4.3 artifacts, the `v44_scout.py` physics path, v3 (`138d109`), v3.1, v3.2, and v3.3 are **not modified** — this is a new file. Every ChatGPT Q1–Q7 disposition v3.1/v3.2/v3.3 satisfied stays satisfied (§12), every v3.1-re-pass repair (§14) and every v3.2→v3.3 repair (§16) stays in force. The v3.3→v3.4 changes are keyed to a reconciliation table (§17). All numeric thresholds are grep-consistent (experimental law #4); **every hard-gate expected value in §9.3 was COMPUTED this session** (MC, `/usr/bin/python3`, seeds 20260708/20260709 — §13); every change from v3.3 is flagged inline as **[v3.4]**. All §0 decisions remain revisable at re-review / audit / Anthony final-say.*
...
## CHANGE SUMMARY — what v3.4 changes vs v3.3 (whole-object re-pass: 1 blocker + 8 majors, deduplicated across three audit lenses to 7 defects)
...
| **39 (40 blocks — §8/§8-Ω powered rerun)** | **1.685** | 2.023 |
...
| **40 / 96 (§8/§8-Ω powered rerun — significance test against 0 only, not a banding config)** | 0.557·sd | **0.2016·sd** | *(fires at \|x\| > t·SE = 1.685·0.2016 = **0.340·sd**)* | *(n/a)* | *(reband-look κ₄₀ = 0.557 − 0.340 = **0.217·sd** [v3.4 — used in the reband-channel accounting, §13])* |
...
**§3.2 ANOMALOUS meaning (retained):** significant wrong-direction current; triggers §8 (primary) or **§8-Ω (omega) [v3.4]** — both **powered** (n_rerun=40).
...
*(State machine, discriminator, disposition table, amendment rule, selftest requirement carry over from v3.3 — including the POWERED STATE-B vanish test: n_rerun = 40 fresh blocks under the cell's predeclared remedy recipe (precond namespace), one-sided t₃₉ = 1.685 against 0, SE = sd·√(1/40+1/64) = 0.2016·sd, significance edge |x| > 0.340·sd, measured power 0.912 ≥ 0.90 at −1×floor(96), residual mislabel ≈ 0.088, hard gate 9. The reband channel is counted in the numbers of record — [v3.4] recomputed for the shared ladder: reband-look RED needs |μ̂₄₀| < κ₄₀ = 0.217·sd, so P(reband lands RED | real −1×floor) ≈ 0.016; channel contribution to gate 5 ≈ 4×10⁻⁵, §13.)*

```
STATE A (entry): cell is ANOMALOUS at M=400 (primary statistic).
  --> Powered rerun: n_rerun = 40 FRESH blocks, remedy recipe, precond namespace.
STATE B (powered vanish test): wrong-sign significance FAILS --> GRID-ARTIFACT
  (mislabel ~0.088 at -1x floor); re-band under the higher-M recipe via amendment;
  [v3.4] omega_roi and occupancy_x are RE-STAMPED from the same 40 rerun blocks
...
**[v3.4] §8-Ω — the omega/occupancy disposition machine (closes D4).** An **omega ANOMALOUS**, a challenged **omega GREEN**, or a challenged **occupancy GREEN** dispositions through the **same machine applied to that statistic**: STATE-B rerun at the **same predeclared powered size, n_rerun = 40 fresh blocks** (t₃₉ = 1.685, SE = 0.2016·sd, edge 0.340·sd — the arithmetic is statistic-agnostic; **power 0.912 ≥ 0.90 at ∓1×floor(96)** for the flagged statistic, asserted in-sim as **gate 9Ω**). A flagged secondary can be rebanded non-vetoing — and the pivot un-blocked — **ONLY** through this powered look + formal amendment (law #1/#9). *(v3.3 §5(iii) routed omega ANOMALOUS to an unspecified "§8-style investigation" with the powered n pinned only for the primary: a ~0.61-power look could formally disposition a real −1×floor omega current as artifact, reband it AMBER, and un-block the pivot through a channel absent from every gate's numbers — the §8 evidence-destruction defect re-entering on the omega branch. Closed: the omega-reband channel is simulated in S8's wrong-sign sub-case and counted in S8's number of record, §13.)*

| Classification | Disposition | Counts toward pivot? |
|---|---|---|
| GRID-ARTIFACT | re-band under higher-M recipe (amendment) — only after failing the POWERED n_rerun=40 vanish test; **[v3.4] secondaries re-stamped from the powered blocks** | yes, at the re-banded result — channel counted (S5/S8) |
| TRUE-OPPOSITE-PHYSICS | Movement-3 contrast in observed direction; retention ≈ 0.91 given surfaced | **no** |
...
| 5 | **P(invalid pivot) — WRONG-SIGN at floor, incl. §8 reband channel, [v3.4] reported per ρ arm incl. ρ ∈ {−1, 0} (belt-only regimes)** | S5 (Δ=−floor), full §8 loop | ≤ 0.02 | **≈ 0.0006 belt-only** (carrier pivot-compatible 0.017 × null⁵ 0.034) **+ reband channel ≈ 4×10⁻⁵** (0.874 ANOM × 0.088 mislabel × 0.016 reband-RED at κ₄₀=0.217 × null⁵); ~0 at ρ > 0 (belt+suspenders) *(v3.2/v3.3's ≈ 0.007 superseded — the omega-RED requirement suppresses the joint)* |
...
| 9 | **§8 vanish-test power at −1×floor (primary)** | S5 STATE-B | ≥ 0.90 | **0.912** (n_rerun=40; MC) |
| **9Ω** | **[v3.4] §8-Ω vanish-test power at ∓1×floor (omega/occupancy branch)** | S8 wrong-sign sub-case | ≥ 0.90 | **0.912** (same statistic-agnostic arithmetic, asserted in-sim) |
...
| **11** | **[v3.4] Occupancy veto failability — P(pivot \| occupancy-only +2×floor in all six cells)** | S10 | ≤ 0.005 | **≈ 1.4×10⁻⁵** (per-cell occupancy GREEN 0.741 at +2×floor; 0.9945 at +3×floor → ≈ 7×10⁻¹⁶, reported) |
...
| TRUE-OPPOSITE-PHYSICS retention | S5 | report | ≈ 0.874 × 0.912 ≈ 0.80 end-to-end at −1×floor |
...
- **Shared-ladder stamp:** a cell whose primary REDs at 16b while omega is AMBER at 16b must NOT be terminal at 16b (extends); both stamps at the terminal rung equal (n_blocks, B_conf) of that rung; κ₄₀ grep = 0.217·sd.
...
| **Anomaly SOP** (§8) | frozen machine + powered STATE-B (n_rerun=40, power 0.912) — **[v3.4] + §8-Ω (same powered machine for omega/occupancy flags; gate 9Ω)** | part of prereg |
...
| **Stale-prose flag (law #4)** | All v3.3 grep items retained (statistic qualifier in every pivot-claim string; roster-parametric claim generation; no hardcoded "six cells/four points"; demotion caveats; n_rerun=40 / t₃₉=1.685 / 0.2016 / 0.340), **plus [v3.4]:** any omega band stamped at a look other than the shared-ladder terminal rung; any "occupancy recorded and unbounded" without banding (must read "banded two-sided and nowhere GREEN"); any missing terminal-occupancy-AMBER caveat; any omega disposition without the powered n_rerun=40; any completeness assertion covering fewer than ALL consumed dicts; κ₄₀ = 0.217; any ρ sweep lacking the {−1, 0} arms; any §1.1 suspenders prose unscoped to non-inverted transduction; **any hard-gate expected value not traceable to a computation** | Update per §13; the selftest grep-check must grep every constant named in this row | part of config_hash / prereg |
...
2. **The operating point (restated for v3.4 — the biggest call in this revision).** Four honest repairs stack: Q4 ceiling, v3.2 magnitude bound, v3.3 omega veto, **[v3.4] shared ladder + blocking omega-INCONCLUSIVE**. Joint pivot-under-null: v3 **0.808** → v3.1 **0.349** → v3.2 **0.148** → v3.3 ≈ 0.11 (superseded arithmetic) → **v3.4 ≈ 0.017** (one-sided omega, ρ_qω=0; ≈ 0.0014 all-two-sided; positive ρ_qω raises it — OC sim is the number of record). **Fork:** accept the ≈ 0.017 / high-INCONCLUSIVE operating point (HQ rec — each reduction closed a leak, every blocking channel is conservative, and the alternative re-opens the gate-8 breach), OR (documented, NOT recommended) demote the omega-INCONCLUSIVE block to a mandatory claim caveat — which re-opens a measured 0.004–0.019 gate-8 straddle — OR raise B_max above 128 to resolve more omega AMBERs. Compute cost of the shared ladder: ≈ +9% blocks/cell.
...
11. **§8 rerun size n_rerun = 40** (unchanged; power table §13).
12. **[v3.4 NEW] Occupancy guard strength.** HQ writes v3.4 with occupancy **banded two-sided + GREEN veto + mandatory caveat for terminal AMBER** (D1″). Alternatives, documented: (a) weaker — record-only with caveat (rejected by HQ: re-creates the D3 costume); (b) stronger — occupancy drives the ladder and its INCONCLUSIVE blocks, symmetric with omega (costs: pivot-under-null collapses further, ≈ 3×10⁻⁴ ballpark, and +compute; buys: a sub-GREEN occupancy current can no longer coexist with a pivot). **HQ rec: the middle option as written** — occupancy has no registered sign, no calibrated sensitivity, and no v4.4-estimand standing; GREEN-veto + caveat guards the super-floor-visible case (the audit's scenario) at negligible null cost. Anthony's call.
...
## 13. Measured / derived operating characteristics (v3.2/v3.3 session numbers retained as history; v3.4 numbers COMPUTED this session — the v3.4 OC sim is the registration-grade instrument)
...
| 5 (belt-only, ρ ∈ {−1, 0}) | Δ=−1×floor carrier (primary: ANOMALOUS 0.874, INCONCLUSIVE 0.107, falsely-RED 0.0195); carrier pivot-compatible 0.0166 | 0.0166 × 0.034 = **0.00057**; **reband channel** ≈ 0.874 × 0.088 × 0.016 × 0.034 ≈ **4×10⁻⁵** (reband-look RED needs \|μ̂₄₀\| < κ₄₀ = 0.557 − 0.340 = 0.217·sd; P ≈ 0.016 at a real −1×floor) → total ≈ **0.0006 ≤ 0.02** |
| 5′ | Δ=−2×floor carrier | carrier pivot-compatible **0 observed** at 4×10⁴ (MC-zero) |
| 8 | S8: Δ_ω=+2×floor(96)=1.114·sd on A,B; carrier omega bands GREEN 0.896 / INCONCLUSIVE 0.086 / RED **0 observed at 4×10⁴** (analytic escape P(omega RED) ≈ Φ((0.067−1.114)/0.25) ≈ 1.4×10⁻⁵/cell at 16b) | ≈ 0.509⁴ × (1.4×10⁻⁵)² ≈ **1.3×10⁻¹¹** one-sided; two-sided branch (carrier GREEN 0.814 / INCONCLUSIVE 0.164 / RED analytic ≈ 8×10⁻⁹ at 32b) ≈ **10⁻¹⁸**. *(Under v3.3's throttled stamp the same construction measures ≈ 0.0075 > 0.005 — the BLOCKER, reproduced before repair.)* |
| 9 / 9Ω | n_rerun=40 powered rerun at ∓1×floor | **0.912** (table below) |
| 11 | S10: Δ_occ=+2×floor all six cells; per-cell occupancy GREEN **0.741** (+3×: **0.9945**); per-cell pivot-compatible (primary RED ∧ omega RED ∧ occ ¬GREEN) **0.156** (+3×: 0.0029) | joint = 0.156⁶ ≈ **1.4×10⁻⁵** at +2×; 0.0029⁶ ≈ **7×10⁻¹⁶** at +3× |

**[v3.4] Wrong-sign omega current (S8 sub-case, Δ_ω = −1×floor, one-sided):** omega terminal ANOMALOUS 0.847, INCONCLUSIVE 0.119, falsely-RED 0.018 → per-cell veto-or-block **0.982**; if cell C sole carrier, P(pivot) ≈ 0.509⁵ × 0.0157 ≈ **5×10⁻⁴** (advisory). The §8-Ω powered rerun retains a real wrong-sign omega at **0.912** given the flag surfaced — the un-block path can no longer launder it at ~0.61 power (D4).

**§8/§8-Ω vanish-test power vs rerun size (v3.3 MC, retained — statistic-agnostic):**

| n_rerun | t₀.₉₅(n−1) | SE factor | power at ∓1×floor |
|---|---|---|---|
| 16 | 1.753 | 0.2795 | 0.613 |
| 24 | 1.714 | 0.2394 | 0.762 |
| 32 | 1.696 | 0.2165 | 0.854 |
| **40 (ADOPTED)** | **1.685** | **0.2016** | **0.912 ≥ 0.90 (gates 9/9Ω)** — mislabel 0.088 |
| 48 | 1.678 | 0.1909 | 0.947 |

**TRUE-OPPOSITE-PHYSICS retention at −1×floor [v3.4 re-measured]:** ≈ 0.874 (anomaly surfaces through the shared ladder) × 0.912 (powered rerun persists) ≈ **0.80 end-to-end**.
...
*End v3.4 draft. Status: DRAFT for ChatGPT re-review + Antigravity re-audit + Anthony final-say. Not registered, not chronicled, no harness bytes edited, not committed. v3 (`138d109`), v3.1, v3.2, v3.3, and `v44_scout.py` (`source_sha 0b65a9ee92b9fe2c`) remain unmodified. The registration-grade v3.4 OC simulation (§9) is the next build phase; its pinned seed (proposal 20260710) is distinct from this session's draft-check seeds (20260708/20260709).*
