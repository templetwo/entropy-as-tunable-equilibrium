...
| `prereg_v44.json` | v2 re-issue: field `occ_reduction` = {name: `lr_asymmetry`, formula: D1″′}, new pins (source_sha, OC-sim hash — which v3.5's MANIFEST never contained), the DIAG-3 additive-diff provenance note | **new sha256** — anchor's staging lane |
